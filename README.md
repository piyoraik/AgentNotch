...
**画面付きの使い方は [docs/HowToUse.md](docs/HowToUse.md)。** 以下はインストールと仕様。
...
承認カードを出せない場面で止まったセッションは、`Notification` フックを登録しておくとノッチに「要応答」とだけ出る。答えるのはターミナルなので、ボタンは端末へのジャンプだけ。
...
- 承認できるのは**ツールの許可 / 拒否**と、`AskUserQuestion` / プラン承認への**選択肢からの回答**まで。自由記述はパネルで受け取らないので、その質問だけターミナルに渡る（`AskUserQuestion` の各質問に出る「その他（自由に書く）」）。
