...
enum Motion {
    /// ノッチの開閉にアニメーションは**置かない**。枠は AppKit、中身は SwiftUI と
    /// 別々のアニメータになるため終わりが揃わず、どう曲線を合わせてもガタついて
    /// 見えた。開閉は即座に切り替える（`NotchWindowController.setExpanded`）。
    ///
    /// 画面の切り替え（一覧 ↔ 詳細 ↔ 承認）。
...
///
/// `MarkLayerView` と同じ理由で、繰り返しは CoreAnimation に載せている。
