...
struct NotchContentView: View {
    ...
    var body: some View {
        ...
        .animation(Motion.expand, value: uiState.isExpanded)
    ...
    private var leftWing: some View {
        VStack(alignment: .leading, spacing: 2) {
            HStack(spacing: 5) {
                ClaudeMarkView(activity: activity, size: 12, introduces: true)
    ...
    private var emptyState: some View {
        VStack(spacing: 10) {
            Spacer(minLength: 8)
            ClaudeMarkView(activity: .idle, size: 34, introduces: true)
                .opacity(0.35)
                ...
                if let summary {
                    ...
                    if showsCost {
                        Text(TokenPricing.format(summary.costUSD))
                            .font(.system(size: 10, weight: .semibold, design: .rounded))
                            .foregroundStyle(ClaudeBrand.clay.opacity(0.9))
                            .rollingNumber()
